...
const Input = ({ name, type = 'text', placeholder, value, onChange, error, show, className = '', ...rest }) => (
  <input name={name} type={type} value={value ?? ''} placeholder={placeholder}
    onChange={onChange}
    className={`w-full px-4 py-3 bg-surface-container-low border border-transparent rounded-lg text-body-md text-on-surface focus:ring-2 focus:ring-primary/20 focus:bg-white transition-all outline-none ${show && error ? '!border-error !ring-error/20' : ''} ${className}`}
    {...rest} />
)
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
            <div className="col-span-1 md:col-span-2">
              <InputRow label="Medicine Name *" error={validationErrors.medicine_name} show={submitted}>
                <Input name="medicine_name" value={form.medicine_name} onChange={e => set('medicine_name', e.target.value)} error={validationErrors.medicine_name} show={submitted} placeholder="e.g. Crocin Advance 500mg" className="text-lg font-bold py-4 px-5 bg-surface-container-highest/30 border-primary/20 border ring-primary/5 ring-1" />
              </InputRow>
            </div>
            ...
            <InputRow label="Quantity (Stock) *" error={validationErrors.quantity} show={submitted}>
              <div className="flex">
                <Input name="quantity" value={form.quantity} onChange={e => set('quantity', e.target.value)} type="number" placeholder="0" error={validationErrors.quantity} show={submitted} className="rounded-r-none border-r-0" />
                <span className={`inline-flex items-center px-4 bg-outline-variant/10 text-on-surface-variant rounded-r-lg font-label-md text-label-md border border-l-0 ${submitted && validationErrors.quantity ? '!border-error' : 'border-outline-variant/20'}`}>Units</span>
              </div>
            </InputRow>

            <InputRow label="Unit Price / MRP (₹) *" error={validationErrors.unit_price} show={submitted}>
              <div className="relative">
                ...
                <input name="unit_price" type="number" step="0.01" value={form.unit_price ?? ''} placeholder="0.00"
                  ...
                  className={`w-full pl-8 pr-4 py-3 bg-surface-container-low border border-transparent rounded-lg text-body-md text-on-surface focus:ring-2 focus:ring-primary/20 focus:bg-white outline-none transition-all ${submitted && validationErrors.unit_price ? '!border-error !ring-error/20' : ''}`} />
              </div>
            </InputRow>
